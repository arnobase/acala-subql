...
export const updatePoolHourData = async (tokenA: MaybeCurrency, tokenB: MaybeCurrency, timestamp: number) => {
	...
	if (!record) {
		...
		// Arno
		/*
		record.token1Open = poolRecord.exchange1
		record.token1Low = poolRecord.exchange1
		record.token1High = poolRecord.exchange1
		record.token1Close = poolRecord.exchange1
		*/
	...
	record.rateToken0Token1 = calculate(poolRecord.token0Amount+"/"+poolRecord.token1Amount)
  	...
  	record.rateToken0Token1Low = (record.rateToken0Token1 < record.rateToken0Token1Low) ? record.rateToken0Token1 : record.raterateToken0Token1LowToken0Token1Min
  	record.rateToken0Token1Open = (record.rateToken0Token1Open == null) ? poolRecord.rateToken0Token1 : record.rateToken0Token1Open
	record.rateToken0Token1Close = poolRecord.rateToken0Token1
	...
	record.rateToken1Token0Open = (record.rateToken1Token0Open == null) ? poolRecord.rateToken1Token0 : record.rateToken1Token0Open
	record.rateToken1Token0Close = poolRecord.rateToken1Token0
